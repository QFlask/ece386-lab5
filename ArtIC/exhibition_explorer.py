...
def display_exhibitions(exhibition_ids: list[int]) -> None:
    ...
    if 'data' in data:
        for i, exhibition in enumerate(data['data']):
            ...
            print(f"   Date: {exhibition.get('start_date', 'N/A')} to {exhibition.get('end_date', 'N/A')}")
...
def main():
    ...
    while True:
        ...
        if search_term.lower() in ['exit', 'quit']:
            print("Thank you for using the Art Institute of Chicago Exhibition Search. Goodbye!")
            sys.exit(0)
